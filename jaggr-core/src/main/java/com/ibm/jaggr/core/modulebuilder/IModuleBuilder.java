...
public interface IModuleBuilder {
	...
	/**
	 * Returns a {@link ModuleBuild} object containing the processed (built) output for the
	 * requested module.
	 * <p>
	 * If an error occurs, this method can throw an exception, or else let an exception that occurs
	 * at a lower level propagate up. How the exception gets handled depends largely on whether or
	 * not development mode is enabled.
	 * <p>
	 * If development mode is not enabled, then the exception is converted to a ServletException and
	 * allowed to propagate to the servlet container. This will result in an error response being
	 * returned to the client.
	 * <p>
	 * If development mode is enabled, then exceptions thrown by this method are handled by creating
	 * a module build containing code to invoke console.error() on the client, specifying the
	 * exception message as the error text, and the aggregated response is flagged as an error
	 * response to prevent caching of the response either on the server or on the client. The
	 * content value which is returned by the define function is an empty string.
	 * <p>
	 * If a builder would like to handle errors in development mode differently, either by providing
	 * partial content or to have more control over what is displayed in the client console, then
	 * this method can return the error content in the build output and use the {@link ModuleBuild}
	 * constructor that allows you to specify an error flag. Specifying true for the error flag will
	 * cause the aggregated response containing this module build to be flagged as an error and the
	 * build output will not be cached either on the server or on the client.
	 * <p>
	 * For AMD modules, if the request attribute specified by
	 * {@link IHttpTransport#EXPORTMODULENAMES_REQATTRNAME} is false, then this method SHOULD return
	 * an anonymous module, however, if the request attribute is true, then this method MUST return
	 * a named module (the first parameter of the define function for the module must be the value
	 * specified by {@code mid}). If the request attribute specified by
	 * {@link IHttpTransport#EXPORTREQUESTEDMODULENAMES_REQATTRNAME} is true, then this method MUST
	 * return a named module if the {@link ModuleSpecifier} for the specified module, which can be
	 * obtained from the {@link ILayer#BUILD_SOURCES_REQATTRNAME} request attribute, equals
	 * {@link ModuleSpecifier#MODULES}, otherwise an anonymous module may be returned.
	 * <p>
	 * For text modules, this method should return a quoted string with any escaping necessary to
	 * represent the string as a JavaScript variable having been performed.
	 * <p>
	 * The module builder may update one or more of the cache key generators in <code>keyGens</code>
	 * by specifying a new list of cache key generators in the returned {@link ModuleBuild} object.
	 * When a new list is provided, the cached cache key generator list for this module, and any
	 * layers containing this module, are updated. If this method returns the same list object
	 * specified by <code>keyGens</code> in the {@link ModuleBuild} , then no updates of the cached
	 * key generators are performed. Most module builders will have no need to update a cached key
	 * generator list once it has been created and should just return <code>keyGens</code> in the
	 * {@link ModuleBuild} object, however, if <code>keyGens</code> is null, then a new cache key
	 * generator list containing only non-provisional cache key generators MUST be provided in the
	 * returned {@link ModuleBuild}.
	 * <p>
	 * A module builder should always return the same sized cache key generator list for any given
	 * resource, and the cache key generators in the list must be the same class, and in the same
	 * sequence, for all responses for the same resource. Note that subclasses of this class may add
	 * their own cache key generators to the result list, so <code>keyGens</code> may be larger than
	 * what this method previously returned because it contains contributions from the subclasses.
	 * If this class wishes to update the cache key generator list, it should not try to include any
	 * of these added cache key generators in the new list. It should only include its own cache key
	 * generators. Subclasses are responsible for detecting that the superclass has updated the list
	 * in the result and then adding their own cache key generators to the new result.
	 *
	 * @param mid
	 *            The module id
	 * @param resource
	 ...
	 * @param keyGens
	 *            List of cache key generators for this module that was obtained by a previous call
	 *            to this method or {@link #getCacheKeyGenerators(IAggregator)}. If null, then the
	 *            {@code ModuleBuild} returned by this method must specify a new list of
	 *            non-provisional cache key generators, otherwise, this method may return a
	 *            {@code ModuleBuild} object that specifies {@code keyGen} for the cache key
	 *            generator.
	 *
