...
@Command(scope = AggregatorCommandProvider.EYECATCHER, name = AggregatorCommandProvider.CMD_VALIDATEDEPS)
public class ValidateDepsShellCommand extends AbstractOsgiCommandSupport {
	...
	@Argument(index = 1, name = "clean", required = false, multiValued = false)
	String clean = null;

	@Override
	protected void exec(AggregatorCommandProvider provider) throws Exception {
		provider._aggregator(new CommandInterpreterWrapper(Arrays.asList(AggregatorCommandProvider.CMD_VALIDATEDEPS, servlet, clean)));
	}
}
