...
@Command(scope = AggregatorCommandProvider.EYECATCHER, name = AggregatorCommandProvider.CMD_PROCESSREQUEST)
public class ProcessRequestShellCommand extends AbstractOsgiCommandSupport {
	...
	protected void exec(AggregatorCommandProvider provider) throws Exception {
		provider._aggregator(new CommandInterpreterWrapper(Arrays.asList(AggregatorCommandProvider.CMD_PROCESSREQUEST, servlet, queryArgs)));
	}
}
