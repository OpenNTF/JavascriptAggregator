...
@Command(scope = AggregatorCommandProvider.EYECATCHER, name = AggregatorCommandProvider.CMD_RELOADCONFIG)
public class ReloadConfigShellCommand extends AbstractOsgiCommandSupport {
	...
	protected void exec(AggregatorCommandProvider provider) throws Exception {
		provider._aggregator(new CommandInterpreterWrapper(Arrays.asList(AggregatorCommandProvider.CMD_RELOADCONFIG, servlet)));
	}
}
