...
@Command(scope = AggregatorCommandProvider.EYECATCHER, name = AggregatorCommandProvider.CMD_SETOPTION)
public class SetOptionShellCommand extends AbstractOsgiCommandSupport {
	...
	@Argument(index = 2, name = "value", required = false, multiValued = false)
	String value = null;

	@Override
	protected void exec(AggregatorCommandProvider provider) throws Exception {
		provider._aggregator(new CommandInterpreterWrapper(Arrays.asList(AggregatorCommandProvider.CMD_SETOPTION, servlet, name, value)));
	}
}
