...
public class CreateCacheBundleShellCommand extends AbstractOsgiCommandSupport {
	...
	protected void exec(AggregatorCommandProvider provider) throws Exception {
		provider._aggregator(new CommandInterpreterWrapper(Arrays.asList(AggregatorCommandProvider.CMD_CREATECACHEBUNDLE, servlet, bundleSymbolicName, bundleFile)));
	}
}
