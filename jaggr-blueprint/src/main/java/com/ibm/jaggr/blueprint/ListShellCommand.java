...
@Command(scope = AggregatorCommandProvider.EYECATCHER, name = AggregatorCommandProvider.CMD_LIST)
public class ListShellCommand extends AbstractOsgiCommandSupport {
	...
	protected String exec(CommandProvider provider) throws Exception {
		return invoke(provider, new CommandInterpreterWrapper(Arrays.asList(AggregatorCommandProvider.CMD_LIST)));
	}
}
